...
class testJoints(unittest.TestCase):
    ...
    def create_circle_body(self, position, dynamic=True):
        ...
        body.CreateFixture(
            shape=self.b2.b2CircleShape(radius=1.0), density=1.0, friction=0.3
        )
        return body
    ...
    def check(self, dfn, joint, prop, joint_prop=""):
        ...
        self.assertEqual(
            a,
            b,
            "Property not equal from definition to joint: %s (dfn %s != joint %s)"
            % (prop, a, b),
        )
    ...
    def pulley_definition(self, body1, body2):
        ...
        dfn.Initialize(
            body1, body2, groundAnchor1, groundAnchor2, anchor1, anchor2, 2.0
        )
        return dfn
    ...
    def do_joint(self, name, init_args):
        ...
        for bodyA, bodyB in itertools.permutations(
            (self.sbody1, self.sbody2, self.dbody1, self.dbody2), 2
        ):
            try:
                dfn = get_dfn(body1=bodyA, body2=bodyB, **init_args)
            except Exception as ex:
                self._fail(
                    "%s: Failed on bodies %s and %s, joint definition (%s)"
                    % (name, bodyA.userData, bodyB.userData, ex)
                )
            ...
            try:
                ...
                self._fail(
                    "%s: Failed on bodies %s and %s, joint creation by kwargs (%s) kw=%s"
                    % (name, bodyA.userData, bodyB.userData, ex, kw_args)
                )
            ...
            try:
                ...
                self._fail(
                    "%s: Failed on bodies %s and %s, joint creation (%s)"
                    % (name, bodyA.userData, bodyB.userData, ex)
                )
            ...
            try:
                ...
                self.world.DestroyJoint(joint)
                self._fail(
                    "%s: Failed on bodies %s and %s, joint checks (%s)"
                    % (name, bodyA.userData, bodyB.userData, ex)
                )
            ...
            try:
                ...
                try:
                    ...
                    self._fail(
                        "%s: Failed on bodies %s and %s, joint simulation (%s)"
                        % (name, bodyA.userData, bodyB.userData, ex)
                    )
            ...
            try:
                ...
                s = sys.exc_info()[1]
                self._fail(
                    "%s: Failed on bodies %s and %s joint deletion (%s)"
                    % (name, bodyA.userData, bodyB.userData, s)
                )
    ...
    def test_rope(self):
        self.do_joint(
            "rope", {"anchorA": (-10, 0), "anchorB": (-0.5, -0.5), "maxLength": 5.0}
        )
