...
Based on the paper:
    ...
    Presented at the 18th International Conference on Process Control, 6/14-17, 2011
...
OpenCV: opencv.willowgarage.com (simple installer for windows here http://www.lfd.uci.edu/~gohlke/pythonlibs/ )
"""
...
class CVObject(object):
    ...
    def bodies_from_image(self, image, flip=True):
        if flip:
            flipped = cv.CreateImage((image.width,image.height), image.depth, image.channels)
...
def main():
    ...
    if display_image:
        ...
        pg_img = pygame.image.frombuffer(image_rgb.tostring(), cv.GetSize(image_rgb), "RGB")
