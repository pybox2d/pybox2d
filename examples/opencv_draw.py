...
from Box2D.b2 import staticBody, dynamicBody, kinematicBody, polygonShape, circleShape, loopShape, edgeShape
...
class OpencvDrawFuncs(object):
    ...
    def _draw_joint(self, joint):
        ...
        x1, x2, p1, p2 = self._fix_vertices((x1*self._ppm,x2*self._ppm,p1*self._ppm,p2*self._ppm))
    ...
    def _draw_polygon(self, body, fixture):
        ...
        vertices=self._fix_vertices([transform*v*self._ppm for v in polygon.vertices])
    ...
    def _draw_circle(self, body, fixture):
        ...
        position=self._fix_vertices([body.transform*circle.pos*self._ppm])[0]
        cv2.circle(self.screen, cvcoord(position), int(circle.radius*self._ppm), self._colors[body.type], 1)
    
    def _draw_edge(self, body, fixture):
        edge = fixture.shape
        vertices=self._fix_vertices([body.transform*edge.vertex1*self._ppm, body.transform*edge.vertex2*self._ppm])
        cv2.line(self.screen, cvcoord(vertices[0]), cvcoord(vertices[1]), self._colors[body.type], 1)

    def _draw_loop(self, body, fixture):
        loop = fixture.shape
        transform=body.transform
        vertices=self._fix_vertices([transform*v*self._ppm for v in loop.vertices])
        v1=vertices[-1]
        for v2 in vertices:
            cv2.line(self.screen, cvcoord(v1), cvcoord(v2), self._colors[body.type], 1)
            v1=v2
